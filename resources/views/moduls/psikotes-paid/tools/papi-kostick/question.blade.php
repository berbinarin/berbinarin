...
@section('content')
    <div class="relative h-11/12 md:min-h-screen flex flex-col items-center bg-gray-100 justify-center">
        <form method="POST"
            action="{{ route('psikotest-paid.papi-kostick.submit', ['psikotestTool' => $psikotestTool->id, 'testPapiKostick' => $testPapiKostick->id, 'questionPapiKostick' => $questionPapiKostick->id]) }}">
            @csrf
...
<div class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
    ...
    <label class="flex-1 rounded-lg p-4 my-8 cursor-pointer relative card"
        ...
        for="answer_a_{{ $questionPapiKostick->id }}">
        <input type="radio" name="answer" id="answer_a_{{ $questionPapiKostick->id }}" value="1" class="hidden" {{ optional($existingAnswer)->answer === 1 ? 'checked' : '' }} required>
        <div class="text-white select-none" style="margin-left: 5px; margin-top: 20px; margin-right: 5px; height: 70px;">
            {{ $questionPapiKostick->a }}
        </div>
        ...
        <div class="absolute bottom-2 right-2 w-5 h-5 rounded-full bg-white flex items-center justify-center"
            ...
            <span class="{{ optional($existingAnswer)->answer === 1  ? '' : 'hidden' }} text-lg checkmark" style="color: #3FA2F6; font-size: 12px;"><strong>&#10003;</strong></span>
        </div>
    </label>
    ...
    <label class="flex-1 rounded-lg p-4 my-8 cursor-pointer relative card"
        ...
        onmouseout="this.style.boxShadow='none';"
        for="answer_b_{{ $questionPapiKostick->id }}">
        <input type="radio" name="answer" id="answer_b_{{ $questionPapiKostick->id }}" value="2" class="hidden" {{ optional($existingAnswer)->answer === 2 ? 'checked' : '' }}>
        <div class="text-white select-none" style="margin-left: 5px; margin-top: 20px; margin-right: 5px; height: 70px;">
            {{ $questionPapiKostick->b }}
        </div>
        <!-- Circle Button for Orange Card -->
        <div class="absolute bottom-2 right-2 w-5 h-5 rounded-full bg-white flex items-center justify-center"
            style="border: 2px solid #FBB03B; margin-top: 3%; margin-left: 85%;">
            <span class="{{ optional($existingAnswer)->answer === 2 && optional($existingAnswer)->answer !== 1 ? '' : 'hidden' }} text-lg checkmark" style="color: #FBB03B; font-size: 12px;"><strong>&#10003;</strong></span>
        </div>
    </label>
</div>
