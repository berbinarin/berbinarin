...
@section('content')
<form method="post" action="{{ route('psikotest-paid.tool.Dass-42.submitAnswer', [$psikotest_paid_test_id, $question_dass]) }}" class="relative min-h-screen h-full w-full flex flex-col items-center justify-center">
    @csrf
    ...
    <div class="relative mx-auto items-center justify-center flex flex-col z-30 inset-0 max-sm:mx-4">
        <p class="text-[#070707] max-sm:text-[15px] text-[17px] font-medium text-center">{{ $question_dass->question }}</p>
        
        <div class="flex space-x-6 mt-10 text-center">
            <label class="flex flex-col items-center cursor-pointer">
                <input type="radio" name="answer" value="0" class="hidden peer" {{ $answer === 0 ? 'checked' : '' }} required>
            ...
            <label class="flex flex-col items-center cursor-pointer">
                <input type="radio" name="answer" value="1" class="hidden peer" {{ $answer === 1 ? 'checked' : '' }} required>
                <div class="w-12 h-12 border border-[#CCCCCC] bg-white rounded-full flex items-center justify-center peer-checked:border-[#3986A3] peer-checked:bg-[#3986A3]">
                </div>
            ...
            <label class="flex flex-col items-center cursor-pointer">
                <input type="radio" name="answer" value="2" class="hidden peer" {{ $answer === 2 ? 'checked' : '' }} required>
                <div class="w-12 h-12 border border-[#CCCCCC] bg-white rounded-full flex items-center justify-center peer-checked:border-[#3986A3] peer-checked:bg-[#3986A3]">
                </div>
            ...
            <label class="flex flex-col items-center cursor-pointer">
                <input type="radio" name="answer" value="3" class="hidden peer" {{ $answer === 3 ? 'checked' : '' }} required>
                <div class="w-12 h-12 border border-[#CCCCCC] bg-white rounded-full flex items-center justify-center peer-checked:border-[#3986A3] peer-checked:bg-[#3986A3]">
                </div>
    ...
    <div class="absolute bottom-10 left-0 right-0 flex items-center mx-auto shadow-md bg-white rounded-lg h-12 sm:w-[70%] w-[90%]">
        <div class="flex items-center ml-6 w-full h-1 relative">
            ...
            <button 
                ...
                {{ $question_dass->question_order >= $totalQuestions ? 'Selesai' : 'Selanjutnya' }}
            </button>
    </div>
