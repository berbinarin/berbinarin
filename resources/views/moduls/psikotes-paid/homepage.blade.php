...
<html lang="en">
    ...
    <body
        x-data="{ open: false, testNumber: null }"
        class="h-screen overflow-hidden bg-cover bg-center bg-no-repeat"
        style="background-image: url('{{ asset("assets/images/psikotes/paid/psikotest-home-bg.png") }}')"
    >
        <!-- Navbar Start -->
        <section>
            <div class="mt-4 flex w-full flex-row items-center justify-between">
                <a href="{{ url("/psikotest-paid/testimoni") }}">
                    <button
                        class="ml-16 rounded-[5px] bg-blue-500 px-[10px] py-3 text-xs font-medium text-white"
                ...
                <button
                    class="mr-16 rounded-[5px] bg-blue-500 px-[10px] py-3 text-xs font-medium text-white"
                    style="font-family: 'Plus Jakarta Sans', sans-serif"
                >
                    Logout
                </button>
        ...
        <div class="my-7 flex w-full items-center justify-center">
            <h1 class="text-2xl font-bold" style="font-family: 'Plus Jakarta Sans', sans-serif">
                Temukan Potensimu - Selamat Datang di Situs Psikotes Kami!
            </h1>
        </div>
        <!-- Tagline End -->
            ...
            <div class="swiper-wrapper mx-4">
                @for ($i = 1; $i <= 21; $i += 4)
                    <div class="swiper-slide !h-auto py-1">
                        <div
                            class="mx-auto grid h-full max-w-[1200px] grid-cols-4 justify-items-center gap-x-3 gap-y-3"
                        >
                            @for ($j = $i; $j < $i + 4 && $j <= 21; $j++)
                                <button type="button" @click="open = true; testNumber = '{{ $j }}'">
                                    <div
                                        class="flex h-[180px] w-[270px] items-center justify-center rounded-[5px] bg-[#3986A3] transition hover:scale-105"
                                    >
                                        <p
                                            class="text-xl font-bold text-white"
                                            style="font-family: 'Plus Jakarta Sans', sans-serif"
                                        >
                                            Test {{ str_pad($j, 2, "0", STR_PAD_LEFT) }}
                                        </p>
                                    </div>
                                </button>
                            @endfor
        ...
        >
            <div class="h-[298px] w-[560px] rounded-lg bg-white p-6 shadow-lg">
                <h2 class="mb-5 text-center text-2xl font-bold" style="font-family: 'Plus Jakarta Sans', sans-serif">
                    Redeem Code
                </h2>
                <h2
                    class="mb-6 text-center text-[15px] text-[#D1D1D1]"
                    style="font-family: 'Plus Jakarta Sans', sans-serif"
                >
                    Enter the redeem code provided to start the test
                </h2>
                <form method="POST" action="{{ url("/psikotest/redeem") }}">
                    ...
                    <div class="flex justify-center">
                        <input
                            ...
                            class="h-[47px] w-[458px] rounded-[10px] bg-[#E1E1E1] px-3 text-[15px] font-bold outline-none placeholder:text-white"
                            style="font-family: 'Plus Jakarta Sans', sans-serif"
                            required
                        />
                    </div>
                    <div class="mt-4 flex justify-center gap-2">
                        <button
                            type="submit"
                            class="rounded-xl bg-[#3986A3] px-28 py-3 text-[15px] font-extrabold text-white"
                            style="font-family: 'Plus Jakarta Sans', sans-serif"
                        >
                            Redeem
                        </button>
    ...
    `,
                    ...
                    customClass: {
                        ...
                        confirmButton:
                            'mt-6 rounded-md bg-[#6083F2] px-[112px] py-[10px] text-[15px] font-extrabold text-white transition hover:bg-blue-600',
                    },
