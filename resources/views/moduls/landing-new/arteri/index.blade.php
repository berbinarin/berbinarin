...
@section("content")
  ...
  <section class="mb-16 flex flex-col overflow-x-hidden px-4 lg:px-14">
    ...
    <div class="mb-4 flex w-full flex-col">
      ...
      <p class="text-center text-base font-normal text-[#606060] lg:text-justify lg:text-lg">
        Temukan solusi psikologi dengan artikel edukasi, tips, informasi terbaru, dan lain-lain
        seputar kesehatan mental untuk membantu menjalani kehidupan yang lebih seimbang dan bermakna
        setiap hari.
      </p>
    </div>
    {{-- menu filter --}}
    <div class="mb-8 flex w-full flex-col items-center justify-between lg:flex-row lg:gap-x-4">
      {{-- filter button --}}
      <div
        class="filter-container mb-4 flex flex-wrap items-center justify-center gap-4 lg:mb-0 lg:flex-grow lg:flex-nowrap lg:justify-start lg:overflow-x-scroll lg:scrollbar-hide"
      >
        <a
          href="{{ route("arteri") }}"
          class="{{ request()->routeIs("arteri") ? "active" : "border-[#606060]/50" }} w-fit rounded-lg border px-8 py-1 font-semibold text-slate-800"
        >
          Semua
        </a>
        {{-- @dump($categories) --}}
        @foreach ($categories as $cat)
          <a
            href="{{ route("arteri.category", ["slug" => $cat->slug]) }}"
            class="{{ isset($category) && $category->id === $cat->id ? "active" : "border-[#606060]/50" }} w-fit text-nowrap rounded-lg border px-8 py-1 font-semibold text-slate-800"
          >
            {{ $cat->name_category }}
          </a>
        @endforeach
      ...
      <div class="flex flex-none items-center justify-end gap-4">
        ...
        <select
          class="w-fit rounded-lg border border-slate-300 bg-white py-2 pe-8 ps-4 text-slate-800 focus:outline-none focus:ring-0 active:outline-none"
          onchange="window.location.href=this.value"
        >
          <option
            value="{{ request()->fullUrlWithQuery(["sort" => "terbaru"]) }}"
            {{ $sort == "terbaru" ? "selected" : "" }}
          >
            Terbaru
          </option>
          <option
            value="{{ request()->fullUrlWithQuery(["sort" => "terlama"]) }}"
