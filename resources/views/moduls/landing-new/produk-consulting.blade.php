...
@section("content")
    ...
    <div class="relative">
      ...
      >
        <div class="max-lg:min-w-auto flex max-w-xl flex-1 flex-col space-y-6 max-sm:space-y-2">
          ...
          >
            <img
              src="{{ asset("assets/images/landing/asset-consulting/hero.png") }}"
              alt="Consulting"
          ...
          <div class="lg:order-4">
            ...
            >
              Layanan yang bertujuan untuk membantu suatu perusahaan/komunitas/organisasi yang
              berkaitan dengan Sumber Daya Manusia. Layanan ini menekankan pada proses consulting
              oleh Human Resource yang berpengalaman agar pengetahuan yang diberikan dapat diterima
              dan dimengerti dengan baik oleh klien.
            </p>
          </div>
        </div>
    ...
    <div class="relative flex flex-col justify-center space-y-4 text-center">
      ...
      <div class="swiperContainer">
        <div class="swiper" id="swiperConsulting">
          <div class="swiper-wrapper">
            @foreach ($layanan_consultings as $layanan_consulting)
              <div class="swiper-slide">
                <div
                  class="flex h-full min-h-[313px] flex-col rounded-xl bg-white bg-opacity-90 px-6 py-4 shadow-md backdrop-blur-sm max-xl:w-[95%] max-lg:min-h-[375px] max-lg:w-full max-sm:min-h-[300px] max-sm:px-4 max-sm:py-3"
                >
                  <p
                    class="min-h-[45px] text-lg font-semibold leading-tight text-black max-lg:min-h-[67px] max-sm:min-h-[40px] max-sm:text-[17px]"
                  >
          ...
          <p class="mt-2 text-left text-[16px] text-[#4F4F4F] max-sm:text-[14px]">
      ...
      <div class="grid w-full grid-cols-1 justify-center gap-5 sm:grid-cols-2">
        @foreach ($bundlings as $bundling)
          <div class="flex flex-col rounded-xl bg-white p-5 text-center shadow-md max-sm:p-3">
            <p
              class="customheight min-h-[43px] text-lg font-semibold leading-tight text-black max-sm:min-h-[43px] max-sm:text-[17px]"
            >
              {{ $bundling["category"] }}
            </p>
            <p
              class="mt-3 text-lg font-semibold leading-none text-[#70787D] max-sm:mt-1.5 max-sm:text-[16px]"
            >
              <del>{{ $bundling["discount"] }}</del>
              <span class="rounded-md bg-[#EF5350] p-1 text-white">10%</span>
            </p>
            <p
              class="mt-3 flex-grow text-[30px] font-semibold leading-none text-[#3986A3] max-sm:mt-1.5 max-sm:text-[22px]"
            >
              {{ $bundling["price"] }}
            </p>
            <img
  ...
  >
    ...
    <img
      ...
      alt="ellipse ornament"
      class="absolute right-0 top-0 z-10 hidden size-[500px] -translate-y-3 translate-x-40 scale-125 lg:block"
    />
    {{-- Ellipse ornamen bottom-left --}}
    <img
      src="{{ asset("assets/images/landing/asset-consulting/Ellipse.png") }}"
      alt="ellipse ornament"
      class="absolute bottom-0 left-0 z-10 hidden size-[500px] -translate-x-40 translate-y-4 scale-125 lg:block"
    />

    {{-- TITLE --}}
    <div class="mb-6 flex w-full items-center justify-center text-center">
      <h1
        class="bg-gradient-to-r from-[#1C4352] to-[#3986A3] to-70% bg-clip-text py-4 text-3xl font-bold text-transparent lg:text-5xl"
      >
        Alur Pendaftaran Consulting
      </h1>
    </div>
    ...
    <div class="relative flex flex-col gap-4 overflow-x-auto py-6 scrollbar-hide lg:gap-8 lg:py-20">
      ...
      <div
        class="relative z-20 hidden justify-center lg:flex lg:translate-x-72 xl:translate-x-44 3xl:translate-x-10 2xl:translate-x-0"
      >
        <img
          src="{{ asset("assets/images/landing/asset-consulting/alur_pendaftaran.png") }}"
          alt="ornament alur pendaftaran"
          class="h-auto lg:max-w-screen-xl"
        />
      </div>
      ...
      <div class="relative z-20 flex flex-col gap-4 lg:flex-row lg:gap-8 xl:gap-6">
        @foreach ($alur_pendaftaran as $step)
          <div class="lg:odd:pt-20">
            <div
              class="flex shrink-0 flex-col items-center justify-center rounded-xl bg-white p-6 text-center shadow-lg lg:w-56 lg:p-4"
            >
              <img
                src="{{ asset($step["link"]) }}"
                alt="icon"
                class="mb-3 hidden size-12 lg:block"
              />
              <img
