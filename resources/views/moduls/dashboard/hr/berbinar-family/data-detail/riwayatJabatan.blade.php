        ...
        @foreach ($records as $record)
            <div>
                <div class="flex items-center gap-3">
                    <p class="text-lg text-blue-400 font-bold">{{ $record->division?->nama_divisi ?? '-' }} -
                        {{ $record->subDivision?->nama_subdivisi ?? '-' }}</p>
                    <span
                        class="py-1 px-2 {{ \Carbon\Carbon::parse($record->date_end)->isFuture() ? 'bg-green-400' : 'bg-red-400' }} text-xs rounded-lg text-white">
                ...
                <ul class="font-semibold pt-5 grid grid-cols-3 gap-x-10 gap-y-5">
                    <li>
                        ...
                        <p class="text-black"> {{ $record->division?->nama_divisi ?? '-' }}
                        </p>
                    </li>
                    <li>
                        <p class="text-gray-400">Sub Divisi</p>
                        <p class="text-black"> {{ $record->subDivision?->nama_subdivisi ?? '-' }}
                        </p>
                    </li>
                    <li>
