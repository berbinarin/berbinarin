                ...
                <div class="bg-white py-4 md:py-7 px-4 md:px-8 xl:px-10 rounded-[24px]">
                    <div class="mt-4 overflow-x-auto">
                        <table id="example" class="display gap-3" style="overflow-x: scroll;">
                            ...
                            <tbody>
                                @foreach ($psikotestIndividu as $psikotest)
                                    <tr id="" class="data-consume">
                                        <td>{{ $psikotest->name }}</td>
                                        {{-- <td>{{ $psikotest->alat_tes }}</td> --}}
                                        <td>ABC DE FG</td>
                                        <td>Rp {{ number_format($psikotest->price, 0, ',', '.') }}</td>
                                        <td class="flex gap-2">
                                            {{-- BUTTON DELETE  --}}
                                            <button type="button"
                                                ...
                                                data-id="{{ $psikotest->id }}">
                                                <i class='bx bx-plus text-white'></i>
                                            </button>

                                            {{-- BUTTON DETAIL  --}}
                                            {{-- <a href="{{ route('psikotest.detail', $psikotest->id) }}"
