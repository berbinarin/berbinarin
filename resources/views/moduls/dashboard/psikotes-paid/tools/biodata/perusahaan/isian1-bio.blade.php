...
<div class="relative h-11/12 md:min-h-screen flex flex-col items-center bg-gray-100 justify-center">
  ...
  <p class="font-bold text-2xl text-center text-gray-800 mt-0 z-20 relative">Tes 22</p>

  <div class="relative text-center z-10 w-3xl mx-auto p-7 mt-8" style="width: 750px;">
    <p class="text-lg mt-7 px-8">
      Sebutkan Kelebihan/Kekuatan dari Pribadi Anda yang Mendukung Tugas Anda Saat Ini (Minimal 3)
    </p>
...
@section('content')
    <div class="relative h-11/12 md:min-h-screen flex flex-col items-center bg-gray-100 justify-center">
        ...
        <form action="{{ route('psikotest-paid.tool.BIODATA_PERUSAHAAN.post') }}" method="post">
            @csrf
            <input type="hidden" name="question_essay_id[{{ $index }}]" value="{{ $question_essay_id }}">
            <div class="relative text-center z-10 w-3xl mx-auto p-7 mt-8" style="width: 750px;">
                <p class="text-lg mt-7 px-8">
                    {{ $question }}
                </p>
                ...
                <div class="mt-6">
                    <textarea name="answer[{{ $index }}]" placeholder="Ketik disini..." class="input-field rounded-lg"></textarea>
                </div>
