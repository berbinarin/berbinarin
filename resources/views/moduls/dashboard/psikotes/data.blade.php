                        ...
                        <tbody>
                            ...
                            <tr id="adminDataPsikotesFree_{{ $user->id }}" class="data-consume">
                                ...
                                <td class="flex gap-2">
                                    ...
                                    {{-- BUTTON EDIT  --}}
                                    <button onclick="toggleModal('edit-modal-{{ $user->id}}')" type="button" class="focus:ring-2 focus:ring-offset-2 mt-0 inline-flex items-start justify-start p-3 bg-yellow-500 hover:bg-yellow-500 rounded">
                                        <i class='bx bxs-edit-alt text-dark'></i>
                                    </button>

                                    {{-- BUTTON DELETE  --}}
                                    <form id="deleteForm-{{ $user->id }}" action="{{ route('dashboard.psikotestfree.data.destroy', $user->id) }}" method="POST">
                                        @csrf
                                        @method("DELETE")
                                        <input type="hidden" name="id">
                                        <button type="button" class="focus:ring-2 focus:ring-offset-2 mt-4 sm:mt-0 inline-flex items-start justify-start p-3 bg-red-500 hover:bg-red-500 focus:outline-none rounded delete-button" data-id="{{ $user->id }}">
                                            <i class='bx bxs-trash-alt text-white'></i>
                                        </button>
                                    </form>

                                    {{-- BUTTON DETAIL  --}}
                                    <a href="{{ route('dashboard.psikotestfree.data.show', $test->id) }}" class="focus:ring-2 focus:ring-offset-2 mt-4 sm:mt-0 inline-flex items-start justify-start p-3 bg-gray-500 hover:bg-gray-600 focus:outline-none rounded">
                                        <i class='bx bx-show text-white'></i>
                                    </a>

                                </td>
                            </tr>
                            <div class="hidden overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none justify-center items-center" id="edit-modal-{{ $user->id}}">
                                <div class="relative w-auto my-6 mx-auto max-w-3xl">
                                    <!--content-->
                                    <div class="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                                        <!--header-->
                                        <div class="flex flex-row justify-between gap-16 w-full p-5 border-b border-solid border-blueGray-200 rounded-t">
                                            <h3 class="text-2xl font-semibold">
                                                Edit Soal
                                            </h3>
                                            <div>
                                                <button class="bg-red-600 h-[30px] w-[30px] rounded-full text-white" onclick="toggleModal('edit-modal-{{ $user->id}}')">
                                                        ...
                                                        <div class="flex flex-row gap-10">
                                                            ...
                                                            <div class="form-group mb-1 flex flex-col">
                                                                <label for="date_of_birth" class="text-blueGray-600 text-base border-0 py-3">Tanggal
                                                                    Tes</label>
                                                                <input required type="date" name="test_date" value="{{ \Carbon\Carbon::parse($test->test_date)->format('Y-m-d') }}" class="p-4 py-3 placeholder-blueGray-300 text-blueGray-600 relative bg-white rounded text-base border-0 shadow outline-none focus:outline-none focus:ring">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <!--footer-->
    ...
    <div class="relative w-auto my-6 mx-auto max-w-3xl">
        ...
        <div class="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <div class="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
                ...
                <button class="p-1 ml-auto bg-transparent border-0 text-black opacity-5 float-right text-3xl leading-none font-semibold outline-none focus:outline-none" onclick="toggleModal('edit-modal-{{ $user->id}}')">
                ...
                <form action="{{ route("dashboard.psikotestfree.data.edit",  $user->id) }}" class="flex flex-col gap-1" method="POST">
                    ...
                    <div class="flex gap-1">
                        <div class="mb-1 pt-0 flex-1">
                            ...
                            <input name="pukul_mulai" id="pukul_mulai" value="{{ $user->name}}" type="time" class="px-3 py-3 placeholder-blueGray-300 text-blueGray-600 relative bg-white rounded text-base border-0 shadow outline-none focus:outline-none focus:ring w-full" required />
                        </div>

                        <div class="mb-1 pt-0 w-full">
                            <label for="gender" class="text-blueGray-600 text-base">Hari</label>
                            <select id="gender" name="gender" placeholder="Placeholder" class="px-3 py-3 appearance-none placeholder-blueGray-300 text-blueGray-600 relative bg-white rounded text-base border-0 shadow outline-none focus:outline-none focus:ring w-full" required>
                    ...
                    <div class="flex gap-1 mb-2">
                        <div class="mb-1 pt-0 flex-1">
                            <label for="pukul_mulai" class="text-blueGray-600 text-base">Pukul Mulai</label>
                            <input name="pukul_mulai" id="pukul_mulai" value="{{ $user->date_of_birth}}" type="time" class="px-3 py-3 placeholder-blueGray-300 text-blueGray-600 relative bg-white rounded text-base border-0 shadow outline-none focus:outline-none focus:ring w-full" required />
                        </div>
                        <div class="mb-6 pt-0 flex-1">
                            <label for="pukul_selesai" class="text-blueGray-600 text-base">Pukul Selesai</label>
                            <input name="pukul_selesai" id="pukul_selesai" value="{{ $user->email}}" type="time" class="px-3 py-3 placeholder-blueGray-300 text-blueGray-600 relative bg-white rounded text-base border-0 shadow outline-none focus:outline-none focus:ring w-full" required />
                        </div>
                        <div class="mb-6 pt-0 flex-1">
                            <label for="pukul_selesai" class="text-blueGray-600 text-base">Pukul Selesai</label>
                            <input name="pukul_selesai" id="pukul_selesai" value="{{ $test->test_date}}" type="time" class="px-3 py-3 placeholder-blueGray-300 text-blueGray-600 relative bg-white rounded text-base border-0 shadow outline-none focus:outline-none focus:ring w-full" required />
                        </div>
                    </div>
                    <!--footer-->
                    <div class="flex items-center justify-end pt-3 border-t border-solid border-blueGray-200 rounded-b">
                        <button class="text-gray-500 background-transparent font-bold  px-6 py-2 text-base outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button" onclick="toggleModal('edit-modal-{{ $user->id}}')">
                            Close
                        </button>
                        <button type="submit" name="submit" class="focus:ring-2  mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-3 bg-primary hover:bg-primary focus:outline-none rounded">
                            <p class="text-base font-semibold leading-none text-white">
    ...
    <div class="relative w-auto my-6 mx-auto max-w-3xl">
        ...
        <div class="bg-white border-0 rounded-lg shadow-lg flex flex-col w-full outline-none focus:outline-none">
            ...
            <div class="flex flex-row justify-between gap-16 w-full p-5 border-b border-solid border-blueGray-200 rounded-t">
                ...
                <div>
                    <button class="bg-red-600 h-[30px] w-[30px] rounded-full text-white" onclick="toggleModal('modal-delete')">
                        <span class='bx bx-x text-3xl leading-none font-medium'></span>
                    </button>
                </div>
            </div>
