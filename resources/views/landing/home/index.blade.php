                ...
                <div style="border-radius: 20px; width: 320px; max-width: 100%; transform: scale(0.8); transform-origin: center center; overflow: hidden; height: 610px">
                    ...
                    @empty
                        <div class="w-full py-12 text-center">
                            <p class="text-base text-[#70787D]">Belum ada konten aktif.</p>
                        </div>
                    @endforelse
                </div>
...
@section("script")
    ...
    <script>
        ...
        var swiperTestimoni = new Swiper('#swiperTestimoni', {
            slidesPerView: 3,
            spaceBetween: 10,
            loop: true,
            speed: 300,
        ...
        // Swiper untuk Konten Sosmed (menggunakan konfigurasi yang sama dengan Produk)
        var swiperSosmed = new Swiper('#swiperSosmed', {
            slidesPerView: 3,
            spaceBetween: 10,
            speed: 300,
            loop: {{ count($contents) > 3 ? "true" : "false" }},
            autoplay: {
                delay: 5000,
                disableOnInteraction: false,
            },
        ...
        // Hentikan autoplay saat hover untuk konten sosmed
        document.getElementById('swiperSosmed').addEventListener('mouseenter', function () {
            swiperSosmed.autoplay.stop();
        });

        document.getElementById('swiperSosmed').addEventListener('mouseleave', function () {
            swiperSosmed.autoplay.start();
        });

        // Custom styling untuk pagination active (optional)
        const style = document.createElement('style');
        style.textContent = `
        #swiperSosmed .swiper-pagination-bullet-active {
            background-color: {{ $theme["learn_more"]["swiper_pagination_active"] ?? "#3886a3" }};
            width: 16px;
            height: 8px;
            border-radius: 10px;
