...
@section('content')
    <form action="{{ route('dashboard.social-media-contents.update', $content->id) }}" enctype="multipart/form-data"
        method="post">
        @csrf
        @method('PUT')
            ...
            <div class="flex h-full w-full flex-col">
                <div class="py-4 md:pb-7 md:pt-12">
                    <div class="">
                        ...
                        <p class="w-3/4 text-disabled">Formulir untuk mengedit konten terbaru Berbinar pada halaman berada
                            website Berbinar..</p>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <select name="platform"
                                ...
                                <option value="instagram"
                                    {{ old('platform', $content->platform) == 'instagram' ? 'selected' : '' }}>Instagram
                                </option>
                                <option value="linkedin"
                                    {{ old('platform', $content->platform) == 'linkedin' ? 'selected' : '' }}>Linkedin
                                </option>
                                <option value="x"
                                    {{ in_array(old('platform', $content->platform), ['x', 'twitter']) ? 'selected' : '' }}>
                                    X</option>
                                <option value="tiktok"
                                    {{ old('platform', $content->platform) == 'tiktok' ? 'selected' : '' }}>TikTok</option>
                                <option value="youtube"
                                    {{ old('platform', $content->platform) == 'youtube' ? 'selected' : '' }}>Youtube
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <input type="number" name="position" placeholder="1"
                                ...
                                value="{{ old('position', $content->position) }}" disabled />
                        </div>
