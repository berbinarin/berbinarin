        ...
        <div class="flex">
            <div class="w-full sm:px-6">
                <div class="py-4 md:pb-7 md:pt-12">
                    <div class="">
                        <p tabindex="0" class="mb-2 text-base font-bold leading-normal text-gray-800 focus:outline-none sm:text-lg md:text-2xl lg:text-4xl">Edit Data Position Requirements</p>
                        <p class="w-2/4 text-disabled">
                            Dashboard >
                            <a href="{{ route("dashboard.position-descriptions.index") }}">Positions Requirements</a>
                            > Edit
                        </p>
                    </div>
                </div>
                <form action="{{ route("dashboard.position-requirements.update", $HiringPosisitonsRequirement->id) }}" class="flex flex-col gap-1" method="post">
                    @csrf
                    @method("PUT")
                        ...
                        <div class="mb-1 flex-1 pt-0">
                            ...
                            <select id="position_id" name="position_id" placeholder="Placeholder" class="placeholder-blueGray-300 text-blueGray-600 relative w-full appearance-none rounded border-0 bg-white px-3 py-3 text-base shadow outline-none focus:outline-none focus:ring">
                                ...
                                @forelse ($HiringPosisitons as $item)
                                    @if ($item->id === $HiringPosisitonsRequirement->position_id)
                    ...
                    <div class="mb-1 w-full pt-0">
                        ...
                        <textarea id="requirement" name="requirement" placeholder="Requirement" required class="placeholder-blueGray-300 text-blueGray-600 relative w-full rounded border-0 bg-white px-3 py-12 text-base shadow outline-none focus:outline-none focus:ring">{{ $HiringPosisitonsRequirement->requirement }}</textarea>
                    </div>
